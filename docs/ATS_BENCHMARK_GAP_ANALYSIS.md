...
The candidate-withdrawal implementation was validated by 41 focused enterprise-control contracts, the complete source regression suite, the frontend unit and production-build checks, and hosted Docker, backup/restore, and authenticated Chromium validation for commit `ec2f95e` ([workflow run 32447407154](https://github.com/lone-wolf-J/EAROS/actions/runs/32447407154)). This confirms that the addition did not displace the compiled recruiter and recovery smoke controls; it is not evidence of a live provider, IdP, or candidate portal deployment.
