...
| Candidate-facing career experience | Branded job discovery, mobile application, consent, accessible/multilingual forms, candidate portal | Public enabled-requisition listing, consent-gated career-site submission, duplicate protection, requisition-owned question definitions, validation, and canonical structured response storage are implemented. | **Covered / activation-ready** | Localization, fully branded career domains, and candidate-portal identity require product/content and IdP decisions. |
| Application workflow and dispositions | Configurable stages, stage history, screening data, withdrawals, rejection/disposition reasons, compliant decision evidence | Tenant-scoped application records, custom pipelines, immutable movement history, approval-gated hire/reject decisions, and decision rationale now include an organization-owned disposition taxonomy that can be attached to a pending decision without bypassing approval. | **Covered** | Candidate withdrawal remains a separate policy design decision; any final outcome still requires independent approval. |
