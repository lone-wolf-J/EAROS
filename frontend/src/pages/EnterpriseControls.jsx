...
export const CONTROL_PANELS = ["overview", "retention", "audit", "data-rights", "notifications", "administration", "integrations"];
...
export default function EnterpriseControls() {
  ...
  const { data: recruiterAlerts = [], mutate: mutateRecruiterAlerts } = useSWR("/ats/notifications/alerts", fetcher);
  ...
  const acknowledgeAlert = async (alertId) => {
    setMessage("");
    try {
      await api.post(`/ats/notifications/alerts/${alertId}/read`);
      await mutateRecruiterAlerts();
      setMessage("In-app alert marked as read.");
    } catch (error) { setMessage(error.response?.data?.detail || "Unable to update the alert."); }
  };
  ...
  return <AppLayout>
    <div className="min-h-full bg-[radial-gradient(circle_at_88%_0%,rgba(45,212,191,.10),transparent_34%),radial-gradient(circle_at_2%_15%,rgba(124,58,237,.12),transparent_30%),#020617] px-5 py-6 md:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        {activePanel === "integrations" && <section className="grid gap-5 xl:grid-cols-[.7fr_1.3fr]"><aside className="rounded-sm border border-amber-400/25 bg-amber-400/5 p-5"><div className="flex items-center gap-2"><PlugZap className="h-4 w-4 text-amber-200" /><h2 className="font-display font-bold text-slate-100">Integration administration</h2></div><div className="mt-4 rounded-sm border border-amber-400/25 bg-amber-400/10 p-3 font-mono2 text-[10px] tracking-wider text-amber-200">{readiness?.integration_administration?.status?.replaceAll("_", " ") || "ADMIN ACCESS REQUIRED"}</div><p className="mt-3 text-sm leading-6 text-slate-500">{readiness?.integration_administration?.secret_handling || "Provider configuration is restricted to workspace administrators."}</p><p className="mt-4 text-xs leading-5 text-amber-100/70">External publishing stays draft-only until an administrator completes provider onboarding and a human approval is recorded.</p></aside><div className="rounded-sm border border-slate-800 bg-slate-950/75 p-5"><h2 className="font-display font-bold text-slate-100">Job-distribution adapters</h2><p className="mt-2 text-sm leading-6 text-slate-500">Catalog entries describe configuration requirements only. They do not claim an active provider connection, account, or publication result.</p><div className="mt-4 grid gap-3 md:grid-cols-2">{readiness?.integration_administration?.job_distribution_adapters?.map((adapter) => <div key={adapter.adapter_id} className="rounded-sm border border-slate-800 bg-slate-900/60 p-3"><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-semibold text-slate-200">{adapter.name}</div><div className="mt-1 font-mono2 text-[9px] tracking-wider text-slate-600">{adapter.provider}</div></div><span className="rounded-sm border border-amber-400/25 px-2 py-1 font-mono2 text-[9px] text-amber-200">{adapter.configuration_state.replaceAll("_", " ")}</span></div><p className="mt-3 text-xs leading-5 text-slate-500">{adapter.notes}</p><div className="mt-3 flex flex-wrap gap-1.5">{adapter.credential_fields.map((field) => <span key={field} className="rounded-sm border border-slate-800 px-1.5 py-1 font-mono2 text-[9px] text-slate-500">{field}</span>)}</div></div>) || <div className="rounded-sm border border-dashed border-slate-800 px-4 py-8 text-center text-sm text-slate-600">No integration catalog is available for this tenant.</div>}</div></div></section>}
      </div>
      {activePanel === "notifications" && <section className="mx-auto mt-5 grid max-w-7xl gap-5 xl:grid-cols-[1.05fr_.75fr_1fr]"><div className="rounded-sm border border-slate-800 bg-slate-950/75 p-5"><div className="flex items-center gap-2"><Bell className="h-4 w-4 text-teal-300" /><h2 className="font-display font-bold text-slate-100">Personal notification preferences</h2></div><p className="mt-2 text-sm leading-6 text-slate-500">These choices are scoped to your EAROS workspace identity and generate an immutable preference-change event. They do not configure a provider or transmit candidate data.</p><div className="mt-5 space-y-2">{[["in_app_enabled", "In-app alerts", "Surface approved work, queue changes, and system notices in EAROS."], ["email_enabled", "Email delivery preference", "Saved as a preference only; no email provider is configured."], ["interview_reminders", "Interview reminders", "Receive future reminder events when delivery is configured."], ["approval_alerts", "Approval alerts", "Notify when governed work enters or leaves a review gate."], ["candidate_activity_alerts", "Candidate activity alerts", "Notify when tracked candidate activity is recorded."]].map(([field, label, detail]) => <label key={field} className="flex items-center justify-between gap-5 rounded-sm border border-slate-800 bg-slate-900/60 px-3 py-3"><span><span className="block text-sm font-semibold text-slate-200">{label}</span><span className="mt-1 block text-xs leading-5 text-slate-500">{detail}</span></span><input aria-label={label} type="checkbox" checked={Boolean(notificationPreferences?.[field])} disabled={!notificationPreferences || savingPreferences} onChange={(event) => saveNotificationPreferences({ [field]: event.target.checked })} className="h-4 w-4 shrink-0 accent-teal-400" /></label>)}</div></div><aside className="rounded-sm border border-amber-400/25 bg-amber-400/5 p-5"><div className="font-mono2 text-[10px] tracking-widest text-amber-200">DELIVERY STATUS</div><div className="mt-3 text-lg font-bold text-amber-100">{notificationPreferences?.provider_delivery_state || "loading"}</div><p className="mt-3 text-sm leading-6 text-amber-50/70">Email and outbound delivery are intentionally inactive until an administrator configures a compliant provider. EAROS retains your choices without claiming a notification has been sent.</p></aside><aside className="rounded-sm border border-slate-800 bg-slate-950/75 p-5"><div className="flex items-center justify-between gap-3"><div><div className="font-mono2 text-[10px] tracking-widest text-violet-300">IN-APP INBOX</div><h2 className="mt-2 font-display font-bold text-slate-100">Your recruiter alerts</h2></div><span className="rounded-sm border border-violet-400/25 bg-violet-400/10 px-2 py-1 font-mono2 text-[10px] text-violet-200">{recruiterAlerts.filter((alert) => alert.status === "unread").length} UNREAD</span></div><p className="mt-2 text-xs leading-5 text-slate-500">Alerts are visible only to the assigned workspace user. Acknowledging an alert never alters the related hiring record.</p><div className="mt-4 max-h-[30rem] space-y-2 overflow-y-auto pr-1">{recruiterAlerts.length ? recruiterAlerts.map((alert) => <div key={alert.recruiter_alert_id} className={`rounded-sm border p-3 ${alert.status === "unread" ? "border-violet-400/25 bg-violet-400/5" : "border-slate-800 bg-slate-900/45"}`}><div className="flex gap-3"><Bell className={`mt-0.5 h-3.5 w-3.5 shrink-0 ${alert.status === "unread" ? "text-violet-200" : "text-slate-600"}`} /><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><span className="text-sm font-semibold text-slate-200">{alert.title}</span><span className="font-mono2 text-[9px] tracking-wide text-slate-600">{alert.alert_type}</span></div>{alert.body && <p className="mt-1 text-xs leading-5 text-slate-500">{alert.body}</p>}<div className="mt-2 flex items-center justify-between gap-3"><span className="text-[10px] text-slate-600">{alert.created_at ? new Date(alert.created_at).toLocaleString() : "just now"}</span>{alert.status === "unread" && <button onClick={() => acknowledgeAlert(alert.recruiter_alert_id)} className="rounded-sm border border-teal-400/30 px-2 py-1 text-[10px] font-semibold text-teal-200 transition hover:bg-teal-400/10">Mark read</button>}</div></div></div></div>) : <div className="rounded-sm border border-dashed border-slate-800 px-4 py-8 text-center text-sm text-slate-600">No in-app alerts assigned to you.</div>}</div></aside></section>}
      {activePanel === "data-rights" && <DataSubjectRequests isAdmin={isAdmin} onMessage={setMessage} />}
    </div>
  </AppLayout>;
}
